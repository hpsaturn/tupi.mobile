...
TupDialog *TupDialog::s_osd = 0;

TupDialog::TupDialog(QWidget * parent) : QWidget(parent), m_timer(0)
{
...
void TupDialog::display(const QString &title, const QString &message, Level level, int ms)
{
    if (message.isEmpty()) 
        return;
    ...
    if (level != None) {
        ...
        switch (level) {
                case Info:
                   {
                     ...
                     m_document->setHtml("<font style=\"font-size:12px;\"><b>&nbsp;&nbsp;" \
                                         + tail);
                   }
                   break;
                case Warning:
                ...
                case Fatal:
                   {
                     QString logo = "icons/fatal_message.png";
                     background = Qt::red;
                     m_document->setHtml("<font style=\"font-size:12px;\"><b>&nbsp;&nbsp;" \
                                         + tail);
                   }
    ...
    move((int) (desktop.screenGeometry().width() - textSize.width()) - 45, 
...
TupDialog *TupDialog::self()
{
    if (! s_osd)
        s_osd = new TupDialog(QApplication::desktop());

    return s_osd;
}

void TupDialog::paintEvent(QPaintEvent *e)
{
    QPainter p(this);
    p.drawPixmap(e->rect().topLeft(), m_pixmap, e->rect());
}

void TupDialog::mousePressEvent(QMouseEvent *e)
{
    Q_UNUSED(e);
...
void TupDialog::drawPixmap(const QBrush &background, const QBrush &foreground)
{
    QPixmap symbol;
