...
struct TupColorAndroidPalette::Private
{
...
TupColorAndroidPalette::TupColorAndroidPalette(const QBrush brush, const QSize size, QWidget *parent) : QWidget(parent), k(new Private)
{
    k->brush = brush;
...
TupColorAndroidPalette::~TupColorAndroidPalette()
{
}

void TupColorAndroidPalette::setSliderPanel()
{
...
void TupColorAndroidPalette::setBaseColorsPanel()
{
...
void TupColorAndroidPalette::initColorsArray()
{
...
void TupColorAndroidPalette::updateSelection(int index)
{
...
void TupColorAndroidPalette::updateMatrixFromSlider(int value)
{
    k->currentLeadColor = value;
    updateMatrix(k->currentBaseColor, true);
}

void TupColorAndroidPalette::updateMatrix(int newColor, bool fromSlider)
{
    ...
    int b = 0;
    int index = 0;
    TupColorAndroidPalette::Color color = TupColorAndroidPalette::Color(newColor);

    for (int i=0; i < k->rows; i++) {
         for (int j=0; j < k->columns; j++) {
              switch (color) {
                      case TupColorAndroidPalette::Red :
                           r = k->currentLeadColor;
                           g = (i*deltaY);
                           b = (j*deltaX);
                      break;
                      case TupColorAndroidPalette::Green :
                           r = (i*deltaX);
                           g = k->currentLeadColor;
                           b = (j*deltaX);
                      break;
                      case TupColorAndroidPalette::Blue :
                           r = (i*deltaX);
                           g = (j*deltaY);
                           b = k->currentLeadColor;
                      break;
                      case TupColorAndroidPalette::White :
...
#ifdef TUP_DEBUG
              // qDebug() << "TupColorAndroidPalette::updateMatrix() - [r,g,b]: " << r << ", " << g << ", " << b;
#endif
              if (r > 255)
                  r = 255;
              if (g > 255)
